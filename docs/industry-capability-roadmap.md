...
| --- | --- | --- | --- | --- | --- | --- |
| B01 | 进行中 | 2026-07-14 | — | 本任务提交 | Mock Host | 首个纵切实现版本化 Manifest、隔离装配、依赖方向与四组合门禁；审计修复补齐相对导入（含 `memory`）依赖门禁、非字符串输入的稳定公开错误和安全诊断上下文；能力包定向测试 39 项、后端单元测试 403 项通过，Ruff、改动范围格式检查及项目 Mypy 通过；不代表 C17 或真实 Core 集成完成 |
