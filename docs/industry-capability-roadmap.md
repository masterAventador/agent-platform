...
| B04 | 🚧 进行中 | 2026-07-16 | — | 分支 `task/b04-media-library`（本任务提交） | Python / TypeScript / Playwright（测试夹具） | Core 反向依赖（C-1）、引用创建 API（H-4）、大小上限零测试（M-1）及第二轮复审跟进（迁移 metadata 漂移守卫、边界测试动态枚举、重复引用受控 409）已按 TDD 修复，验证命令与细节见第 5 节 B04 修复记录；三层授权/`video.*` 权限域（待 C17）、真实腾讯 STS、Core C14 审计、成片下载与生产验收仍未完成 |
