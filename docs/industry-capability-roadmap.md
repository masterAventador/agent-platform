...
| --- | --- | --- | --- | --- | --- | --- |
| B01 | 进行中 | 2026-07-14 | — | 本任务提交 | Mock Host | 首个纵切实现版本化 Manifest、隔离装配、依赖方向与四组合门禁；三轮审计修复补齐相对导入（含 `memory`）、模块段边界和动态导入严格原语门禁（禁止受保护业务目录使用 `importlib`、`__import__` 与 `builtins.__import__`，不模拟执行或数据流，插件加载限 bootstrap/注册表层），以及稳定公开错误、安全诊断上下文、canonical 路由、与能力身份绑定且由 Host 独占的资源命名空间、从 Core 权限与事件定义派生的资源命名空间及真实 API 路由根集中保留契约、控制字符拒绝、架构协议一致性门禁及内置供应商无关精确协议快照；能力包定向测试 115 项、后端单元测试 479 项通过，Ruff、改动范围格式检查及项目 Mypy（152 个源码文件）通过；静态依赖门禁不作为安全沙箱，设备/本地任务协议、受认证 Tauri IPC、C17 与 B17 真实组合回归仍未完成，因此保持进行中 |
