...
| B04 | 🚧 进行中 | 2026-07-16 | — | 分支 `task/b04-media-library`（本任务提交） | Python / TypeScript / Playwright（测试夹具） | Core 反向依赖（C-1）、引用创建 API（H-4）、大小上限零测试（M-1）及第二轮复审跟进已按 TDD 修复；2026-07-17 已合并最新 main 并将迁移改号 0024→0029（详见第 5 节 B04 记录）；三层授权/`video.*` 权限域接线（C17 已就位）、真实腾讯 STS、Core C14 审计接入、成片下载与生产装配验收仍未完成 |
