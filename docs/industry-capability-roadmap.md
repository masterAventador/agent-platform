...
2026-07-16 两轮修复追记（分支推进至 `6bdd8e8`，详细记录见分支上的本条目）：第一轮解除 Core 反向依赖（模型/仓储/路由迁入能力包，新增 `BackendCapabilityRegistration` 注册协议与 bootstrap 组合根，AST 边界守卫扩展）、补引用创建/查询 API 与大小上限精确拒绝测试；第二轮复审跟进修复迁移 metadata 与运行时同源（新增 autogenerate 漂移守卫，顺带对齐 Core 既有声明漂移）、AST 白名单动态化覆盖 workers/sandbox/observability、重复引用改受控 409 且 InMemory 与 SQL 仓储语义对齐。两轮门禁 211 定向 + 891 全量 + ruff/mypy 全绿。C14 已合入主线，迁移改号（0024→0025 指向 C14 的 0024）与 rebase 待执行；三层授权仍待 C17，真实 STS 待云凭据。
...
| B04 | 🚧 进行中 | 2026-07-16 | — | 分支 `task/b04-media-library`（两轮修复至 `6bdd8e8`） | Python / TypeScript / Playwright（测试夹具） | Core 反向依赖已解除、引用 API 与迁移漂移守卫已补；三层授权（待 C17）、真实 STS、迁移改号与生产装配见第 5 节 B04 记录 |
