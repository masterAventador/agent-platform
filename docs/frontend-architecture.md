...
> 状态：已确认的实现基线  
> 确认日期：2026-07-12  
> 补充确认：2026-07-14（Tauri 主交付、AI 中台 Core 与客户能力包）
...
- 以 Tauri App 作为普通用户主入口，并按企业授权装配视频剪辑、自动运营等可选能力。
...
Tauri 桌面 App 是普通用户的主要交付入口。Web 构建继续保留，用于复用同一套 React 业务代码、运行自动化测试、内部调试和未来可选的企业管理端；普通用户不需要在 App 和网页之间切换完成同一个业务流程。
...
### 2.4 Core 与可插拔 Feature

前端分为永久 Core Feature 和可选行业 Feature：

- Core：认证、企业、数字员工、任务、审批、知识、Skill、Tool、产物、审计和设置；
- `video-studio`：素材、Timeline、模板、云剪辑、成片和发布准备；
- `social-operations`：平台账号、内容发布、私信、微信、朋友圈、曝光和本地 RPA；
- Solution Pack：客户专属工作流、提示词、品牌和业务参数，只配置通用 Feature，不复制页面。

可选 Feature 只能依赖 Core 的公开组件、API、Schema、平台事件和 `PlatformAdapter`。Core 禁止导入可选 Feature，`video-studio` 与 `social-operations` 禁止互相导入内部实现。
...
每个可选 Feature 必须导出稳定模块清单，声明 `capability_id`、路由、菜单、权限、懒加载入口和必要桌面能力。App 层只消费模块清单，不写客户名称判断。
...
- 客户差异通过模块清单、服务端能力授权和 Solution Pack 配置实现，禁止复制 Feature 或维护客户专属页面分支；
- 可选 Feature 的公开入口必须稳定，其他模块禁止从其内部目录深层导入。
...
导航根据服务端返回的“部署已安装、企业已授权、用户有权限”能力清单裁剪。无权限的入口不显示，直接访问受限路由时仍必须由路由守卫和后端共同校验。前端模块清单和隐藏菜单都不能替代后端授权。
...
Tauri 只承载客户端原生适配，不内置云端 Agent 后端。自动运营能力包可以引入本地 RPA Sidecar，但必须由能力清单显式启用，并具备进程隔离、最小权限、IPC 认证、代码签名、版本锁定、自动升级、暂停、取消和人工接管。Core-only 或未授权客户不得下载或启动该 Sidecar。
...
- Tauri 本地可信页直连云端 HTTPS API 时，后端只对明确的 Tauri 自定义协议源开放带凭据 CORS；正式环境 Session Cookie 强制使用 `Secure` 与 `SameSite=None`，禁止以通配源配合凭据；
- 登录、注册、退出、Session 过期和当前用户恢复统一由 `features/auth/` 管理；
- 前端需要支持后端以后开启邮箱验证，而不重写整个认证流程；
- OIDC / 企业 SSO 后续作为并行登录方式接入，不替换本地邮箱账号能力；
- Tauri 的长期 Token、企业密钥等高敏凭据必须使用系统安全存储，不能放入 `localStorage`；Demo 阶段的“记住账号密码”按用户明确选择写入 App 私有数据目录，不调用系统钥匙串，并在界面清楚提示其设备级存储范围；
...
- 下载或打开任务产物；
- 权限拒绝和会话过期；
- Core-only、Core+视频、Core+自动运营以及正式客户交付组合；
- 可选能力未安装、未授权和无用户权限时的导航、路由和接口拒绝。
...
- Web 构建产物用于自动化测试、内部调试或可选企业管理端；第一阶段不作为普通用户主入口公开部署；
- Tauri 复用同一前端构建，增加签名、Capabilities、更新通道和平台安装包；
- 客户安装包由同一主干和受版本控制的交付 Profile 生成，禁止手工修改客户专版源码；
- Playwright、OCR 等大体积 Sidecar 支持签名的可选打包或按需下载，未授权时不得安装或启动；
- Vite 构建开关只决定是否包含模块代码，最终企业授权仍由运行时后端能力清单决定；
...
- Tauri 文件、凭据、通知和更新能力；
- 完整权限、可访问性和前端观测；
- 能力注册表、企业授权清单、Core-only 组合和客户交付 Profile。
...
- 更完整的协议兼容、灰度和故障恢复；
- 根据真实需求决定移动 App 技术栈；
- 按客户需求接入 `video-studio`、`social-operations` 和对应 Tauri Sidecar。
...
这些选择不得改变本文确认的核心边界：React + TypeScript + Vite、Ant Design / Ant Design X、Tauri 作为普通用户主交付、服务端状态与客户端状态分离、平台事件解耦、Web/Tauri 业务代码复用、平台差异统一经过 `PlatformAdapter`、Core 与可选 Feature 单向依赖、Playwright 作为 E2E 框架。
