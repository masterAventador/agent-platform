...
| C07 | 🚧 进行中（收口轮完成，待主代理验收） | 2026-07-16 | — | 分支 `task/c07-knowledge-runtime`（已 merge origin/main，收口轮改动在工作区待主代理验收提交） | 召回参数/重排/元数据过滤全链路（领域+snapshot 版本化+API+前端编辑器+Worker 生效+迁移 0027）、真实检索响应契约修复（document_keyword/include_metadata）、未注册 provider 永久错误、文档生命周期与引用闭环 Playwright E2E、真实 RAGFlow v0.25.6 集成验收均已完成并通过门禁；真实实例重排端到端验证与 knowledge.retrieved 多检索事件键为遗留，详见第 4 节 C07 收口轮记录 |
