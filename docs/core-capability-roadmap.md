...
| 审计与可观测性 | 已完成 | 平台审计协议、不可抵赖链、查询导出、脱敏、Trace/Metrics/Logs、告警规则和运维入口已落地；未来模块按同一协议接入 | C14 |
...
| C14 | 已完成 | 2026-07-16 | 2026-07-16 | 本任务提交 | 通用平台审计协议、租户隔离查询、JSONL 导出、仓储边界脱敏、哈希链完整性校验、保留清理、OpenTelemetry Trace/Metrics/Logs、Collector 配置、告警规则和前端运维入口完成；Worker、队列、模型网关、RAGFlow 和 Sandbox 关键操作已接入低基数指标。验证：`cd backend && uv run pytest tests/contract/test_health.py tests/contract/audit/test_audit_events.py tests/unit/observability/test_telemetry.py tests/integration/database/test_migrations.py -q`（31 passed）；`cd backend && uv run pytest tests/unit/observability/test_operational_metrics.py -q`（2 passed）；`cd backend && uv run pytest tests/unit/workers/test_main.py tests/unit/knowledge/test_ragflow_client.py -q`（42 passed）；`cd backend && uv run pytest tests/unit/queue/test_redis_run_queue_claim.py tests/unit/queue/test_redis_run_queue_dlq.py tests/integration/queue/test_redis_run_queue.py tests/integration/queue/test_run_dead_letters.py -q`（24 passed、6 skipped）；`cd backend && uv run pytest tests/unit/observability/test_telemetry.py tests/unit/workers/test_runtime_adapters.py tests/unit/workers/test_main.py tests/unit/runtimes/test_deep_agent_runtime.py -q`（57 passed）；`cd backend && uv run pytest tests/unit/llm/test_litellm_gateway.py tests/unit/sandbox/test_local_controller_provider.py tests/unit/sandbox/test_controller.py tests/unit/sandbox/test_manager.py tests/unit/workers/test_sandbox_janitor.py -q`（86 passed）；`cd backend && uv run ruff check . && uv run mypy`；`uv run --project backend python infra/observability/test_config.py`；`cd frontend && pnpm exec tsc -b --noEmit && pnpm exec oxlint`；`cd frontend && pnpm exec vitest run src/features/operations/api/audit.test.ts src/features/operations/api/queries.test.tsx src/features/operations/pages/AuditObservabilityPage.test.tsx src/app/App.test.tsx --reporter=dot`（28 passed）；正式 Playwright 审计入口隔离栈通过 |
| C07、C09-C13、C15-C20 | 尚未开始 | — | — | — | 按第 4 节逐项更新 |
