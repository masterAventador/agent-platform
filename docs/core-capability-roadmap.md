...
| 多轮会话 | 待集成 | 会话/消息/任务关系、追加输入、错误重试、权限隔离、活跃任务后自动续跑派生与会话内直接取消均已实现并通过正式 E2E；实现分支 `task/c05-conversation-closure` 待主代理终审合入后转已完成 | C05 |
...
| 审计与可观测性 | 进行中 | C14 已合入主线：审计协议、哈希链、脱敏、保留清扫、Trace/Metrics/Logs、告警规则和运维入口；隔离验收栈完整回归与 S7 范围确认前不标记完成 | C14 |
...
| 验证项 | 当前结果 |
| --- | --- |
| 后端 Pytest | C14 合入后默认环境 1045 通过、40 跳过、0 失败；条件跳过均明确标注缺少 PostgreSQL、Redis、MinIO、破坏性本地 Docker 沙箱或显式真实腾讯云 COS 凭据，不计作对应真实依赖验收通过 |
...
| Ruff | 通过 |
| Mypy | 185 个源码文件通过（C14 合入后） |
| 前端 Vitest | C14 合入后 43 个测试文件、195 项测试通过；新增动态输入表单、数字/字符串枚举提交、可选布尔字段不静默提交 false、必填布尔字段未触碰时按 false 提交、正则/日期/数值倍数/数组长度与唯一性校验、浏览器不兼容 pattern 受控错误、数组 item 类型解析、动态文件幂等重试、零字段动态空输入、员工编辑器 Schema JSON 配置与非法 JSON 拦截、结构化输出展示、metadata-only/空/legacy 输出 Schema 兼容、员工编辑器默认空输出 Schema 和任务详情结构化结果回归，并保留既有同步双击互斥、上传后 Run 失败补偿、任务意图换键、Skill 生命周期和幂等请求头契约 |
| 前端 Lint | 通过 |
| 前端 Typecheck | 通过 |
| 前端 Build | 通过 |
| Playwright Web 业务回归 | C14 合入后 20 项完整回归通过（含审计观测新用例；随机隔离端口 + 独立 Compose 项目，测试容器、网络和卷已销毁）；本轮暴露并修复 C05 遗留的 fixture 元数据注册缺陷（dead-letter 场景 `runs.conversation_id` 外键找不到 `conversations` 表），5 个独立进程 fixture 已统一显式 `load_database_models()`；注意宿主机 Homebrew postgresql@14 常驻 5432，正式回归必须使用 `PLAYWRIGHT_*` 隔离端口避免误连；另有正式随机 MVP Profile 3 项真实 Worker/Sandbox 纵切通过 |
...
| C07 | 🚧 进行中 | 2026-07-16 | — | 分支 `task/c07-knowledge-runtime`（已 rebase 到 main，两轮修复完成至 `3adf02d`） | 高危缺陷（Worker 装配、断连语义、畸形响应、事件幂等、批量补偿）与错误分类中级缺陷已修复并复审；重排配置、元数据过滤全链路、引用 E2E 仍未完成，详见第 4 节 C07 记录 |
| C14 | 🚧 进行中（已合入主线，待最终验收） | 2026-07-16 | — | 本任务提交（merge 合入） | 三轮复审完成：S1-S5 阻断项、M1/M2 中风险项、G1-G3 规格缺口均修复，最终代码质量复审 PASS 后以 --no-ff 合入 main；标记完成前仍需：隔离验收栈完整回归、S7 哈希链局限的验收范围确认（无 HMAC/外部锚定，防不了全量重写 DB 的攻击者）；L3-L5 follow-up 见第 4 节 C14 记录 |
