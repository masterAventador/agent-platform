...
| 审计与可观测性 | 进行中 | C14 已合入主线：审计协议、哈希链、脱敏、保留清扫、Trace/Metrics/Logs、告警规则和运维入口；S7 已通过 HMAC 密钥签名加固解除（剩余密钥管理面归 C18）；隔离验收栈完整回归前不标记完成 | C14 |
...
2026-07-16 主线收口启动：经用户批准与 C07/C09/C17 并行，实现分支 `task/c05-conversation-closure`，范围即上述两项收口（自动续跑派生 + 会话内取消体验），预计无新迁移。
