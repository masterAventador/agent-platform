...
| C07 | 🚧 进行中 | 2026-07-16 | — | 分支 `task/c07-knowledge-runtime`（已 rebase 到 main，本任务提交完成高危缺陷修复轮与错误分类中级缺陷修复） | 复审阻断项（Worker 装配、断连语义、畸形响应、事件幂等、批量补偿、前端交互、契约同步）已按 TDD 修复并通过后端 875 项与前端定向测试；复审中级缺陷（Provider 4xx/业务错误码误分类为瞬态导致烧满重投）已修复，新增永久错误 `knowledge_provider_rejected`（后端 883 项通过），两条低级遗留见第 4 节 C07 记录；重排配置、元数据过滤全链路与引用 E2E 仍未完成 |
