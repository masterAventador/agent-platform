...
- FastAPI、SQLAlchemy、HTTPX、Redis 和结构化日志已接入 OpenTelemetry；API 请求数、耗时和 Trace/Logs 具备 correlation_id 关联；
- `infra/observability/alert-rules.yml` 固化 API、Worker、队列、模型网关、RAGFlow、沙箱、客户端和审计写入失败的指标名与告警规则；当前已有 API 指标真实采集，后续生产面按同名指标接入；
