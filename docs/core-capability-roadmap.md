...
实现记录（阶段二 —— 用量/成本记录，完成定义第 3 条；2026-07-17，本任务提交，分支 `task/c16-usage-recording`，全程先 RED 后 GREEN）：

- **红线：纯观测面，只记录不干预**。捕获点对模型调用行为零影响——记录落库失败、归属缺失都绝不改变 Run 结果、不拦调用。预算/配额/限流/告警一行未碰（留阶段三）。
- **设计 A（捕获点）= LangChain 公开扩展点 `AsyncCallbackHandler` 的实例级 local callbacks**（`workers/model_usage_capture.py::ModelUsageCaptureHandler`），在 `ComposedRuntimeResolver._compose` 里 per-run 经 `attach_usage_capture`（`model_copy` 而非原地改 `.callbacks`，避免注入模型跨 run 累积 callback）挂到解析出的 `BaseChatModel` 上。**选它而非包装器**：Deep Agents/LangGraph 会 `bind_tools`，包装器返回的 `RunnableBinding` 会绕过包装层捕获；实例级 callback 在 `bind_tools` 后仍随同一实例触发（有单测 `test_attach_usage_capture_does_not_mutate_original_and_survives_bind_tools` 钉住），覆盖流式（usage 在最后一个 chunk 的 `usage_metadata`）、ReAct 工具循环（每次物理调用各一条、同归属）、错误（`on_llm_error`）。**记账粒度 = 每次 LangChain 物理生成一条**；`ChatOpenAI.max_retries` 由 openai 客户端在单次调用内部重试、对回调不可见，故一次逻辑调用只落一条，不因重试重复计数。
- **设计 B（费用）= 平台定价表，非 LiteLLM**。**真实验证（真实 LiteLLM v1.86.2 容器 + 标准 ChatOpenAI + 回调）实测**：LiteLLM 只把费用放 HTTP 响应头（`x-litellm-response-cost-original`，随附 raw 头证据），**响应体里没有费用**；标准 `ChatOpenAI` 回调的 `on_llm_end` 只拿到 `token_usage`/`usage_metadata`/`model_name`，**拿不到任何响应头**（`contains 'cost'? False`）。要取那个头必须绕过 LangChain 公开模型（自写 openai 调用或挂 httpx 事件钩子）= 侵入，否决候选①。候选③（LiteLLM spend API）按租户 Key 聚合、**归属不到 run/employee**，只能对账不能作归属来源，否决。故采用候选②：`platform/model_gateway/pricing.py::ModelPricingTable` 按 alias 声明价 × token 计算，**金额全整数 nano-USD**（价格以 nano-USD/每百万 token 整数表达，`ModelAliasPrice` 拒绝浮点），未知 alias / 未知 token → 费用 `None`（未知），**绝不塌缩成 0**（0 与未知语义不同，阶段三预算会读它）。默认表为 `general-purpose` 配可调**占位价**（$0.40/$1.20 每 1M in/out，`cost_source=platform_pricing_table` 标明来源），使费用管线可端到端验证；**这是平台声明的占位价、不是真实供应商账单**，alias 背后真实模型由 LiteLLM 路由、平台不感知，运维接真实供应商后须按实调整。
- **设计 C（归属注入）**：回调默认只知 model+usage；tenant/run/employee 由 `_compose` 闭包从 `run` 注入。领域列 `run_id`/`employee_id` 可空（未知），tenant_id 非空（隔离边界）。归属缺失时标 unknown 不丢记录（`ModelUsageRecord` 允许 None 归属 + 仓储 `test_null_attribution_record_persists_and_is_queryable`）。**`model_alias` 只记 provider-neutral alias、绝不记 LiteLLM 回传的真实模型名**（`test_records_provider_neutral_alias_not_real_model_name` + 查询 API 不回 response_metadata，平台协议不泄露）。
- **交付纵切**：迁移 `20260716_0038`（`model_usage_records` 表，down_revision=0037，合并后单头；tenant_id FK CASCADE，run/employee 刻意不加 FK 让计费历史比 run 生命周期长，`(tenant_id, recorded_at)` 索引）；领域 `platform/model_gateway/usage.py`（记录/枚举/端口/查询类，`__post_init__` 强制费用整数、error 必带分类、success 不带、latency 非负、费用与来源同时可知或同时未知）；仓储 `infrastructure/database/repositories/model_usage.py`（写入 + 租户隔离 keyset 分页查询 + 有界 `prune_older_than`；`SessionModelUsageRecorder` 每条独立 session 落库，不跨调用内存缓冲——进程崩溃至多丢正在进行的那一次、不丢已落库）；查询 API `GET /api/v1/model-gateway/usage`（`models.usage.read` 读、按 tenant 严格隔离、时间范围 + base64 keyset 游标分页、费用以整数字符串或 null 返回）；生产装配 `workers/main.py` 注入 `SessionModelUsageRecorder`（`test_production_worker_assembly_wires_model_usage_recorder` 背书，防 C07 型装配缺口）；保留清扫接入 `model-gateway-controller` 循环（`SessionModelUsagePruner` + 配置 `model_usage_retention_days`=90 等，清扫失败绝不终止对账循环）。
- **失败矩阵逐项覆盖**：① 记录落库失败绝不拖垮 Run 且非静默（`_persist` 捕获→logger.error + 可选信号，绝不抛出；`test_persist_failure_does_not_raise_and_emits_signal`）；② 错误调用也记录（`on_llm_error` → error 分类 + 延迟、无 token/费用，只记异常类名不记异常文本防凭据/prompt 泄露）；③ 流式（`test_streaming_usage_from_final_chunk_metadata_with_no_llm_output`）；④ 重试（记账粒度 = 物理生成，openai 内部重试不可见故不重复计数，已在代码与本记录说明）；⑤ 工具循环（`test_tool_loop_records_each_physical_call_to_same_run`）；⑥ 多副本并发写（真实 PG `test_concurrent_writes_all_persist_without_collision` 50/50 落库，随机 PK 无唯一键、不阻塞）；⑦ 租户隔离（仓储 + 契约 `test_tenant_isolation`，跨租户读空/404）；⑧ 长期无界成本（`prune_older_than` + 配置驱动间隔，真实 PG `test_prune_deletes_old_keeps_recent_and_is_bounded`）；⑨ 进程重启恢复（每次调用结束即落库、不做跨调用内存缓冲，取舍已记录）；⑩ 平台协议不泄露（查询 API 只回平台用量模型，`_assert_no_framework_leak` 断言无 response_metadata/model_name 等）。
- **变异验证（装上→红→还原→绿，均实测）**：① 让落库失败传播（`_persist` 改 re-raise）→ `test_persist_failure_does_not_raise_and_emits_signal` 转红 → 还原绿；② 归属串号（记录 run_id 硬编码为固定 UUID）→ `test_concurrent_runs_do_not_cross_attribution` 转红 → 还原绿；③ 费用用浮点（删 `ModelUsageRecord` 的费用整数守卫）→ `test_cost_must_be_int_or_none_never_float` 转红 → 还原绿；④ 剥离租户隔离（查询去掉 `tenant_id` 过滤）→ 仓储 `test_cross_tenant_query_returns_nothing` + 契约 `test_tenant_isolation` 转红 → 还原绿。
- **真实边界验收（核心门禁，非 mock）**：`tests/integration/model_gateway/test_real_litellm_usage_capture.py`——真实 `ChatOpenAI` 经**真实 LiteLLM v1.86.2 容器**发一次真实 HTTP 推理，回调在真实 `on_llm_end` 触发、经 `SessionModelUsageRecorder` 落进**真实 PostgreSQL**，再直接查库断言。**实测证据**：`alias=general-purpose prompt=1 completion=3 total=4 latency_ms=31 outcome=success cost_nanousd=4000 cost_source=platform_pricing_table run=True emp=True`（cost 4000 = 1×400 + 3×1200，与定价表一致；归属精确命中）。用一次性随机名/端口/只绑 127.0.0.1 的 LiteLLM stub + PG 容器，验后 `docker rm -f`；未触碰 `agent-platform-dev` 常驻栈（13 服务全程 healthy）。
- **验证命令与采集条件**（**采集条件影响计数**——`TEST_DATABASE_URL` 指真实 PG 时真实依赖用例才执行）：`uv run pytest tests/unit tests/contract -q` → **1565 passed**（不需 PG，含本阶段新增 pricing 8 + usage 域 6 + 捕获 8 + 组合装配 1 + 生产装配 1 + 契约查询 5 + controller 用量清扫 2）；`TEST_DATABASE_URL=<真实 PG> uv run pytest tests/integration/model_gateway tests/integration/database tests/integration/bootstrap -q` → **87 passed**（含 autogenerate 漂移守卫认到新表、用量仓储、真实 PG 并发写 50/50、迁移 head=`20260716_0038`）；`TEST_DATABASE_URL + TEST_LITELLM_URL + TEST_LITELLM_KEY` 三者齐 → 真实 LiteLLM 用量验收 1 passed（缺任一即 skip 不假绿）；`uv run ruff check .`、`uv run mypy`（258 文件 0 错误）。
- **已知缺口 / 遗留项（阶段二）**：① 默认定价为占位价，真实账单归因（供应商真实单价、多 alias）随阶段五第 8 条落地；② 费用取自平台定价表而非 LiteLLM 响应头（已实证候选①不可达），若未来需与 LiteLLM 账单对账，用其 spend API 做二次校验（不作归属来源）；③ 前端用量页面属阶段五，本轮纯后端；④ 用量变更本身不产生 C14 审计事件（观测记录不是治理动作；模型配置/预算变更的审计在阶段一/三/五），符合阶段划分。
- **⚠️ 跨单位换算（阶段三预算比较前必读，规格复审点名、主代理合并时补写）**：用量费用与定价表用 **nano-USD**（本阶段刻意取比预算更细的单位，容纳单次低 token 调用的 sub-micro 精度）；阶段一租户预算 `budget_microusd` 用 **micro-USD**。**1 micro-USD = 1000 nano-USD**。阶段三拿用量（nano）比预算（micro）时**必须先对齐单位**——直接 `sum(cost_nanousd)` 比 `budget_microusd` 会把预算当成大 1000 倍，约 $0.001 用量就误判预算耗尽。建议聚合后统一到 micro-USD（`sum(nano) // 1000`，量级已大、换算无损）。该换算关系已同步写进 `platform/model_gateway/pricing.py` docstring。
- **本阶段明确不含**（留阶段三/四/五，不得据此宣称 C16 完成）：预算/配额执行、限流、用量告警、fallback（阶段三）、评测（阶段四）、前端页面 + 第 8 条账单归因验收（阶段五）。**C16 整体仍 `🚧 进行中`**。
