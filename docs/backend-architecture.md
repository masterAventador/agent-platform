...
- 租户虚拟 Key 采用「派生而非存储」：明文与其 SHA256 摘要都由 `AGENT_PLATFORM_MODEL_GATEWAY_KEY_SECRET` + `tenant_id` + `key_version` 确定性派生，数据库只保存 `key_version` 与 `retired_key_version` 两个整数，不含任何由凭据派生的材料；Key 明文绝不进入 API 响应、日志、审计 metadata 或异常文本。该派生密钥在 staging/production 缺失或使用开发弱密钥时 fail-closed；把它纳入 KMS/Vault 托管与轮换属 C18；
- 轮换即一次 desired 变更：同一事务内递增 `key_version`、记下待回收的 `retired_key_version`、递增策略 revision 并入队 reconcile 命令；版本必须先落库再触达网关，否则崩溃会在网关侧留下无人回收的孤儿 Key。未回收上一版本前禁止再次轮换（`model_gateway_key_rotation_in_progress`）。轮换与撤销立即生效、不为在途 Run 保留旧凭据：在途 Run 已持有的旧 Key 被删除/阻断后其后续模型调用会失败，不静默降级；
- 只有 Controller 持有 LiteLLM master key；API 只写 desired 状态，Worker 只派生本租户 Key。Worker 每次运行按租户解析凭据，策略缺失、已撤销、尚未对账、Key 未签发或 alias 越权时一律 fail-closed，绝不回退应用级共享 Key；该解析只读平台数据库、不调用网关管理接口，因此 LiteLLM 管理面故障不波及存量 `active` 租户的推理；
