...
class SandboxManager:
        ...
        if action == "delete_then_provision":
            try:
                await provider.delete(
                    sandbox_id=self._required_sandbox_id(lease), lease_id=lease.id
                )
            except Exception:
    ...
    async def delete(self, *, lease_id: UUID, scope: SandboxScope) -> SandboxLease:
        async with self._unit_of_work_factory() as unit_of_work:
            ...
            await unit_of_work.commit()
        try:
            await provider.delete(sandbox_id=self._required_sandbox_id(lease), lease_id=lease.id)
        except Exception:
            await self._persist(deleting.mark_error("delete_failed"))
            raise
        deleted = deleting.mark_deleted()
        await self._persist(deleted)
        return deleted
            ...
            try:
                if lease.sandbox_id is not None:
                    await self._provider(lease.provider).delete(
                        sandbox_id=lease.sandbox_id, lease_id=lease.id
                    )
            except Exception:
                await self._persist(deleting.mark_error("cleanup_failed", now=timestamp))
                continue
            await self._persist(deleting.mark_expired(now=timestamp))
    ...
    async def _provision(
        ...
        *,
        ...
        try:
            ...
            await self._persist(lease.mark_error("acquire_failed", now=now))
            raise
        try:
            ...
            failed = lease
            try:
                await provider.delete(sandbox_id=provisioned.sandbox_id, lease_id=lease.id)
            except Exception:
                error_code = "backend_validation_cleanup_failed"
                failed = lease.activate(provisioned.sandbox_id, now=now)
            await self._persist(failed.mark_error(error_code, now=now))
            raise
        active = lease.activate(provisioned.sandbox_id, now=now)
        await self._persist(active)
    ...
    async def _reconnect_provider(
        ...
        try:
            provisioned = await provider.reconnect(sandbox_id=sandbox_id, lease_id=lease.id)
            self._backend_validator.validate(provisioned.backend)
            if provisioned.sandbox_id != sandbox_id:
                raise ValueError("供应商重连返回了不同的 sandbox_id")
        except Exception:
            await self._persist(lease.mark_error("reconnect_failed"))
            raise
        recovered = lease
        if lease.status is SandboxLeaseStatus.ERROR:
            recovered = lease.activate(sandbox_id)
            await self._persist(recovered)
        return self._environment(recovered, provisioned)

    async def _persist(self, lease: SandboxLease) -> None:
        async with self._unit_of_work_factory() as unit_of_work:
            await unit_of_work.leases.update(lease)
            await unit_of_work.commit()
