...
class CapabilityHostError(RuntimeError):
    """Base error for capability composition failures."""
...
class UnsatisfiedCoreProtocolError(CapabilityHostError):
    """The host cannot satisfy a versioned public Core dependency."""
...
class CapabilityHost:
    """Validated in-process registry of installed capability manifests.

    唯一的装配入口：bootstrap 在进程启动时按显式安装清单 install，
    平台其余模块只读取安装结果，禁止运行期动态装载。
    """
    ...
    def __init__(self, *, core_protocols: Mapping[str, str]) -> None:
        ...
        for protocol_id, protocol_version in core_protocols.items():
            try:
                ...
                raise ValueError("invalid host Core protocol") from None
    ...
    @property
    def installed_manifests(self) -> tuple[CapabilityManifest, ...]:
        return tuple(
            self._manifests[capability_id] for capability_id in sorted(self._manifests)
        )

    def manifest_for(self, capability_id: str) -> CapabilityManifest | None:
