"""生产腾讯 COS 对象核验/预签名预览/清理 Provider（M-3）。

选型说明：COS 的 ETag 对多段上传不是内容 MD5，也不存在服务端 sha256；
核验使用「上传时由客户端写入的自定义元数据头 `x-cos-meta-sha256` + 服务端
可信的 Content-Length」与草稿声明比对。真实网络行为由 TEST_COS_* 门禁覆盖，
本文件用注入的 COS 客户端验证参数、失败语义与元数据缺失的失败关闭。
"""
...
@pytest.mark.asyncio
async def test_verifier_reads_trusted_size_and_declared_sha_metadata() -> None:
    client = FakeCosClient()
...
async def test_verifier_fails_closed_on_missing_object_or_missing_metadata() -> None:
    ...
    # 缺少 sha256 元数据的对象不可核验：返回空摘要，交由服务层按元数据不一致失败。
