...
- C01-C20 未全部完成前，不进入视频剪辑、自动运营等行业能力包的正式开发，用户明确授权的独立调研、竞品分析和方案设计除外。
...
- Core 路线图 C01-C20 全部完成并通过 Core-only 回归后，才进入行业能力包的正式开发；用户明确授权的独立调研、竞品分析和方案设计不受此门禁限制；
- 行业能力包必须按照路线图 B01 → B17 的顺序逐项实施，不得先做展示性页面、跳过公共安全能力或为单个客户复制一套旁路实现；
- 同一时间最多只能有一个业务条目处于 `🚧 进行中`；开始实现前必须先更新条目状态和开始日期；
