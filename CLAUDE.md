...
- 子代理等待外部服务就绪（容器健康、端口可达、任务完成等）必须有界：设定明确的重试上限或时间上限，超限即走受阻分支记录证据并继续其余工作，禁止无限轮询；主代理在处理其他事务的间隙应顺手巡检活跃 worktree 的最近写入时间与 Docker 容器异常状态（如 Restarting/OOM），发现停滞立即干预；
