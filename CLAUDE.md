...
- 默认由主代理在同一会话内按两张独立清单亲自执行规格复审与代码质量复审；只有用户当次明确要求启用子代理时，才允许把复审拆给子代理；
- 如用户明确授权使用复审子代理，复审子代理必须独立读取实现、测试和生产装配，不能把开发代理总结、已有绿测或前一位复审结论当作事实；主代理负责审查范围、证据核验和最终放行，不得直接转述子代理的 PASS。
...
- 当前已派出的收尾/复审子代理完成后，默认不再派发新的子代理；Core 与业务包任务由主代理串行亲自实现、审查、验证和提交，优先从 C 系列继续推进；
- 只有用户当次明确要求“派子代理、并行代理、多人审查”或外部平台强制需要独立执行单元时，才允许启用子代理；历史并行规则不再构成自动派发授权；
...
- Core 以路线图声明的前置依赖为准推进；默认由主代理串行亲自实施，优先继续 C 系列，不再按并发槽位主动启动多个 Core 条目；
- 同一时间默认只允许一个 Core 条目处于 `🚧 进行中`；只有用户当次明确要求并行时，才允许最多两个互不构成直接依赖、且主要修改边界可隔离的 Core 条目并行，每项必须使用独立分支/工作树、独立任务提交和独立测试证据；
