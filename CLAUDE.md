...
每个新会话首次进入本项目时，必须完整阅读一次 `docs/backend-architecture.md` 和 `docs/frontend-architecture.md`，同一会话内无需在每次设计、实现或修改前重复读取。仅当对应文档在会话期间发生变化，或当前上下文中已无法确认其内容时，才需要重新读取。这两份文档分别是后端与前端架构、运行时边界和工程结构的权威说明；如果代码、临时方案或其他文档与其冲突，应先向用户说明并确认是否修改架构基线，禁止自行引入另一套技术体系。
