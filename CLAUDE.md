...
- 长命令必须取得明确的进程或 session 标识，最多每三十秒读取一次新增输出；进程退出后必须立即处理退出状态和结果，禁止停在工具返回处不再推进；
- 子代理连续五分钟没有阶段消息时，按上一节要求主动检查其状态、工作区改动、测试进程和最近有效输出；连续十分钟无进展必须中断接管或重新拆分，不得只等待被动通知；
- 等待子代理期间，主代理必须继续执行不冲突的审查、测试、冲突预检或合并准备；确实没有本地工作可做时，单次等待不得超过六十秒，超时后立即重新核查执行状态；
...
- 跨重启、上下文压缩或网络中断后，脏工作树只证明改动存在，不能证明原执行者已经退出；为该任务派遣新代理前，必须同时核对协作代理列表、Codex 会话/根回合日志、相关进程或命令 session、最近源码写入时间，并至少经过两个连续观察窗口确认没有未知写入；任一证据显示旧根回合、旧子代理或延迟工具调用仍可能继续时，必须冻结新写入并等待或精确终止旧执行单元，禁止让两个代理接管同一工作树；
- 每个并行任务必须保持唯一写入者；恢复后若发现文件在当前唯一写入者未操作时变化，当前代理必须立即停止生产代码编辑并报告，主代理先定位迟到写入来源、确认工作树稳定和重新读取完整差异，再决定恢复，禁止以补丁冲突方式继续抢写；
- 产品层强制中断、网络断开或 Codex 会话暂停期间，代理无法自行执行或唤醒；恢复后必须明确说明中断时段没有实际进展，并从最近检查点继续，不得把暂停时间计作工作时间或声称后台仍在推进。
...
- 执行编译、静态检查或 Rust 单元测试前，必须先判断本轮是否会解析新依赖、更新锁文件或访问网络。凡是新增/变更 `Cargo.toml`、缺少本机缓存、需要访问 crates.io，或其他明确会下载依赖的 `cargo build`、`cargo check`、`cargo test`，第一次就必须在 Codex 沙箱外执行，禁止先在受限网络沙箱内试跑再重跑；只有依赖与锁文件已齐备、能够使用 `--locked --offline`（或等价离线方式）完成时，才允许在沙箱内执行。相同原则适用于 pnpm、uv、Docker 等需要联网拉取依赖或镜像的命令；
- 测试命令执行前还必须检查用例是否会监听或绑定本机端口、创建真实 TCP/UDP/Unix Socket、启动本地服务器、调用系统服务、拉起 GUI，或依赖沙箱外子进程/IPC。包含这些行为的测试第一次就必须在 Codex 沙箱外执行；只有纯内存、纯文件、Mock 传输或不触发受限系统能力的单元测试才允许留在沙箱内。禁止用沙箱拒绝本机端口、进程或系统服务访问后的失败结果判断产品代码有问题；
- 用户已为本项目日常开发和测试明确授予持续的沙箱外执行权限。凡属于当前任务范围内的依赖下载、编译、测试、本机 loopback/端口、IPC、无头自动化，以及隐藏且无 Dock 的真实 Tauri 验收，Codex 应直接按最小范围发起沙箱外命令，不再先通过聊天重复索要口头授权；如运行平台仍强制显示审批，则使用与命令类别匹配的最小审批前缀并正常提交审批，不得绕过拒绝。该持续授权不扩展到远程部署、破坏性删除、无关项目、公开网络暴露或干扰用户正常应用；
